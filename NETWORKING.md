# Networking Tracker

**Purpose:** Track every connection, invite note, response, and follow-up conversation.  
When a response comes in, paste it here and Claude will help craft the next message.  
**Last Updated:** 2026-08-23

---

## HOW THIS WORKS

1. **Invite sent** → logged here with the exact note used
2. **Response received** → tell Claude what they said, Claude drafts the reply
3. **Conversation continues** → log each exchange; Claude helps keep it productive
4. **Goal** → every conversation should move toward: referral, advice, introduction, or job intel

**Status codes:**
- 🟡 PENDING — invite sent, no response yet
- 🟢 REPLIED — they responded, conversation active
- 🔵 WARM — ongoing productive convo
- ✅ CONVERTED — led to referral / intro / job lead
- ❌ NO RESPONSE — 2+ weeks, move on

---

## CONNECTIONS — FP&A Professionals

### 1. Théodore Bros
- **Role:** Financial Analyst | FP&A & Investment | TotalEnergies Dubai
- **Profile:** https://www.linkedin.com/in/theodore-bros/
- **Date sent:** 2026-08-23
- **Status:** 🟡 PENDING
- **Invite note sent:**
  > "Hi Théodore, I actually just applied for the FP&A & Economist role at TotalEnergies Dubai and came across your profile. Curious how you found the FP&A environment there — energy sector finance seems like a different beast. Would love to connect."
- **Their response:** —
- **Next message:** —
- **Notes:** Applied to TotalEnergies this session. If he responds, angle toward: what the hiring process looks like, who the hiring manager is, what skills they weight.

---

### 2. Nadiya Simran
- **Role:** Financial Analyst | FP&A & Revenue/Pricing Analytics | LIVBNB Dubai
- **Profile:** https://www.linkedin.com/in/nadiya-simran-a8709014b/
- **Date sent:** 2026-08-23
- **Status:** 🟢 REPLIED
- **Invite note sent:**
  > "Hi Nadiya, the revenue & pricing analytics angle at LIVBNB caught my eye — most FP&A roles I see in Dubai are pure reporting/variance. How did you carve out that pricing scope within FP&A? Would love to connect and hear more."
- **Their response:**
  > "Hi Ashnad, thanks for reaching out! My pricing and revenue analytics scope at LIVBNB evolved from my broader financial analysis responsibilities. I started analyzing occupancy, ADR, revenue trends, market/competitor pricing and seasonality, and gradually took ownership of pricing decisions and revenue optimization using tools such as PriceLabs. What I particularly enjoyed was that it went beyond reporting — the analysis directly influenced commercial decisions and revenue outcomes. It also allowed me to leverage my earlier financial operations and reconciliation experience in a more commercially focused environment. Happy to connect and discuss this further. I'd also love to learn more about your FP&A experience and current work."
- **Next message (draft):**
  > Haha wait, Northern Trust, were you in Bangalore? EY was in the same campus there, might have walked past each other at some point.
  >
  > The pricing feeding into actual commercial decisions bit stood out to me. Most FP&A I see here is just reporting and variance. I'm in a similar spot to where you started actually, financial ops and reporting right now, trying to get into more analytical work. How did you end up making the Dubai move?
- **Notes:** Responded warmly, asked about Ashnad's experience. Personal connection: Northern Trust Bangalore may be same campus as EY. Next conversation: how she landed Dubai role, what the transition from ops to analytical looked like.

---

### 3. Devika Menon
- **Role:** Manager, FP&A | Dubai Aerospace Enterprise
- **Profile:** https://www.linkedin.com/in/devika-menon-82825475/
- **Date sent:** 2026-08-23
- **Status:** 🟡 PENDING
- **Invite note sent:**
  > "Hi Devika, I'm transitioning into FP&A in Dubai and came across your profile. FP&A in an aerospace/government entity sounds like a very different set of pressures than commercial FP&A — would love to connect and learn what that looks like in practice."
- **Their response:** —
- **Next message:** —
- **Notes:** Senior connection — government/semi-government FP&A. If she responds, focus on: what skills matter most in FP&A here, how she'd evaluate a candidate transitioning from accounting ops.

---

### 4. Shraddha Chichani
- **Role:** FP&A | MIS | Ex-Landmark Group Dubai
- **Profile:** https://www.linkedin.com/in/shraddha-dixit/
- **Date sent:** 2026-08-23
- **Status:** 🟡 PENDING
- **Invite note sent:**
  > "Hi Shraddha, your FP&A & MIS experience at Landmark Group stood out — retail distribution finance is where I'm focused right now. How did you find the MIS reporting side compared to the core FP&A work? Would love to connect."
- **Their response:** —
- **Next message:** —
- **Notes:** Retail FP&A peer — Al Tayer is the same space. If she responds, explore: Landmark Group culture, what MIS vs FP&A looks like day-to-day, hiring at Landmark.

---

### 5. Anil Kumar, CFA
- **Role:** Head of Finance | FP&A | M&A | Treasury | Dubai
- **Profile:** https://www.linkedin.com/in/anilkumar528/
- **Date sent:** 2026-08-23
- **Status:** 🟢 REPLIED (accepted — no note was sent with request)
- **Invite note sent:** None — request sent without a note
- **Their response:** Accepted request
- **First message (sent 2026-08-23):**
  > Hi Anil, thanks for accepting. Should have sent a note with the request, my bad.
  >
  > I'm in finance in Dubai trying to move into FP&A. Came across your profile and your range stood out, FP&A, M&A, treasury all in one career. Would love to hear how you built that if you're ever open to a chat.
- **Notes:** Senior leader, CFA. First message just opens the door, no ask yet. If he responds, ask: what he looks for when hiring junior FP&A, how he'd position someone with accounting ops background.

---

### 6. CA Hardik Patel
- **Role:** Financial Analyst | FP&A | Financial Modelling | IFRS/GAAP | Diamond DMCC Dubai
- **Profile:** https://www.linkedin.com/in/ca-hardik-patel-46062020b/
- **Date sent:** 2026-08-23
- **Status:** 🟡 PENDING
- **Invite note sent:**
  > "Hi Hardik, the combination of financial modelling, IFRS/GAAP and transfer pricing on your profile is interesting — most FP&A profiles I see in Dubai lean one way or the other. How does that blend play out day-to-day? Would love to connect."
- **Their response:** —
- **Next message:** —
- **Notes:** Direct peer. If he responds, explore: what financial modelling looks like in practice at his level, how he broke into FP&A, what the Dubai market rewards.

---

### 7. CA Adrian Machado
- **Role:** FP&A & Financial Modelling | CEO's Office, Sobha | Ex-KPMG Big 4
- **Profile:** https://www.linkedin.com/in/adrian-m1221/
- **Date sent:** 2026-08-23
- **Status:** 🟡 PENDING
- **Invite note sent:**
  > "Hi Adrian, your path from KPMG Big 4 into FP&A at Sobha's CEO office is almost exactly what I'm working toward — I'm post-EY, currently in finance ops, pushing toward FP&A. How did you make that jump? Would love to connect and hear your take."
- **Their response:** —
- **Next message:** —
- **Notes:** Strongest strategic connection — Big 4 → FP&A trajectory mirrors Ashnad's. If he responds: this is the most important conversation to develop. Ask specifically how he bridged the gap, what he'd do differently, whether he knows of openings.

---

### 8. Nandini Vijayan
- **Role:** Senior Financial Analyst | Finance Business Partner | FP&A | ACCA
- **Profile:** https://www.linkedin.com/in/nandinivijayan/
- **Date sent:** —
- **Status:** ⬜ NOT YET SENT
- **Invite note:** Hi Nandini, 21 mutual connections and you're in FP&A in Dubai — seemed like a natural connect. I'm moving from finance ops into FP&A and your profile came up multiple times. Would love to connect.
- **Their response:** —
- **Notes:** Highest mutual connection count in database (21). Senior FA/Finance BP, ACCA. Top networking target.

---

### 9. Muhammad Danish Ali
- **Role:** Senior Finance Manager | FP&A | Oracle FCCS/Fusion | Power BI | ERP Implementation
- **Profile:** https://www.linkedin.com/in/danishali3/
- **Date sent:** —
- **Status:** ⬜ NOT YET SENT
- **Invite note:** Hi Danish, the FP&A plus ERP implementation combination on your profile stands out — most people have one or the other. I'm in finance in Dubai building toward FP&A and wanted to connect.
- **Their response:** —
- **Notes:** 14 mutual connections, 33K followers. ACCA, MCSI, PMP. Great visibility contact.

---

### 10. Sabith Mohmed
- **Role:** Financial Analyst | Dubai Holding | FP&A | Financial Modelling | CMA | MBA
- **Profile:** https://www.linkedin.com/in/sabithmh/
- **Date sent:** —
- **Status:** ⬜ NOT YET SENT
- **Invite note:** Hi Sabith, Financial Analyst at Dubai Holding is exactly the kind of role I'm targeting. 13 mutual connections — seemed like a good time to reach out and connect.
- **Their response:** —
- **Notes:** 13 mutual connections. Dubai Holding is a top target employer. Direct peer.

---

### 11. Mudasir Aslam
- **Role:** Senior FP&A Analyst | CMA | FMVA | Financial Analysis & Reporting
- **Profile:** https://www.linkedin.com/in/mudasir-aslam/
- **Date sent:** —
- **Status:** ⬜ NOT YET SENT
- **Invite note:** Hi Mudasir, your CMA and FMVA combination alongside FP&A experience stood out. I'm in finance in Dubai moving toward FP&A and wanted to connect with people doing the work.
- **Their response:** —
- **Notes:** 12 mutual connections, 10K followers. Based in Jeddah not Dubai but strong FP&A insights. CMA validates Ashnad's certification path.

---

### 12. Haider Javed
- **Role:** CFA | Portfolio Manager | FP&A | Private Equity | M&A
- **Profile:** https://www.linkedin.com/in/haider-javed-cfa-8a5bb099/
- **Date sent:** —
- **Status:** ⬜ NOT YET SENT
- **Invite note:** Hi Haider, the FP&A and private equity mix on your profile is something I don't see often. I'm in finance in Dubai working toward FP&A roles and your background caught my attention. Would love to connect.
- **Their response:** —
- **Notes:** 11 mutual connections. CFA. Good senior contact for long-term network building.

---

### 13. Subodh Lamichhane
- **Role:** CA | Financial Modelling | IFRS | Budgeting and Forecasting | Emaar | Dubai Opera
- **Profile:** https://www.linkedin.com/in/subodh-lamichhane/
- **Date sent:** —
- **Status:** ⬜ NOT YET SENT
- **Invite note:** Hi Subodh, FP&A at Emaar is a benchmark role in Dubai. I'm in finance here transitioning into FP&A and your profile came up through mutual connections. Would love to connect.
- **Their response:** —
- **Notes:** 10 mutual connections, 7K followers. CA, Emaar confirmed. High-value employer connection.

---

### 14. Jeff Landers, ACA
- **Role:** ACA | Finance Professional | Dubai
- **Profile:** https://www.linkedin.com/in/jeff-landers-aca-7bb604aa/
- **Date sent:** —
- **Status:** ⬜ NOT YET SENT
- **Invite note:** Hi Jeff, you keep coming up as a mutual connection across several FP&A profiles I've come across in Dubai. I'm in finance here working toward FP&A roles. Would love to stay connected.
- **Their response:** —
- **Notes:** Bridge person — appears as mutual for 8+ people in the database. Connecting here unlocks large portion of Dubai FP&A network via 2nd degree.

---

### 15. Zaynah Aboobaker
- **Role:** Finance Professional | Dubai
- **Profile:** https://www.linkedin.com/in/zaynah-aboobaker/
- **Date sent:** —
- **Status:** ⬜ NOT YET SENT
- **Invite note:** Hi Zaynah, you show up as mutual across a lot of FP&A profiles I've found in Dubai. I'm in finance here and moving toward FP&A. Would love to connect.
- **Their response:** —
- **Notes:** Bridge person — appears as mutual for 7+ people in the database. Second most important bridge after Jeff Landers.

---

### 16. Shobhit Gupta
- **Role:** CA | FMVA | FP&A Analyst | Multi-Entity Forecasting & Variance Analysis | Dubai
- **Profile:** https://www.linkedin.com/in/shobhit-gupta-fpa/
- **Date sent:** —
- **Status:** ⬜ NOT YET SENT
- **Invite note:** Hi Shobhit, the career break you used to build FMVA and portfolio projects before landing an FP&A role in Dubai is one of the more honest paths I've come across. Would love to connect.
- **Their response:** —
- **Notes:** Most similar trajectory to Ashnad — used 9-month intentional break to earn FMVA + build portfolio projects, then landed Dubai FP&A. Direct proof the strategy works.

---

### 17. Anjali C
- **Role:** ACCA | Finance Analyst | FP&A & Business Partnering | Ex-EY | Olam
- **Profile:** https://www.linkedin.com/in/anjali-c-9a5a54121/
- **Date sent:** —
- **Status:** ⬜ NOT YET SENT
- **Invite note:** Hi Anjali, the EY to FP&A path you've taken is almost exactly what I'm working on. Post-EY, in finance ops right now, pushing toward FP&A in Dubai. Would love to connect.
- **Their response:** —
- **Notes:** Ex-EY audit India → UAE FP&A. Very similar background to Ashnad. Good peer for transition insights.

---

### 18. Ayush Sitani
- **Role:** Finance Business Partner | FP&A | dubizzle (Bayut) | Ex-Goldman Sachs | ADNOC
- **Profile:** https://www.linkedin.com/in/ayushsitani/
- **Date sent:** —
- **Status:** ⬜ NOT YET SENT
- **Invite note:** Hi Ayush, I just applied for a Finance role at dubizzle and came across your profile. Would love to connect and hear what the finance environment there is like.
- **Their response:** —
- **Notes:** Finance BP at dubizzle — Ashnad applied to dubizzle Associate Commercial Analyst role. Insider perspective on the team and hiring.

---

### 19. CA Tony Thomas
- **Role:** CA | Associate Finance Manager | ex-EY | FP&A | Budgeting & Forecasting | IFRS
- **Profile:** https://www.linkedin.com/in/ca-tony-thomas-04852b209/
- **Date sent:** —
- **Status:** ⬜ NOT YET SENT
- **Invite note:** Hi Tony, ex-EY in finance in Dubai here too. Your connections across the space suggest you know the market well. Would love to stay connected.
- **Their response:** —
- **Notes:** 6K+ connections himself — excellent bridge/connector. ex-EY, same transition path.

---

## RECRUITERS OUTREACH

### 1. Talha Sajjad — Synthify HR Solutions
- **Contact:** talha.sajjad@synthify.ae
- **Date sent:** 2026-08-23
- **Status:** 🟡 PENDING
- **Follow-up date:** 2026-09-06 (2 weeks)
- **Message sent:**
  > Subject: CV | Financial Analyst / FP&A | Dubai
  >
  > Hi Talha,
  >
  > I'm looking to move into a Financial Analyst or FP&A role in Dubai and wanted to send my CV across.
  >
  > Currently at Promotech Advertising handling financial reporting, month end close and I led a full ERP migration from Tally to Odoo. Before this I was a Tax Analyst at EY in Bangalore. M.Sc. in Financial Economics, work across Excel, Power BI, SQL and Python. Current title is accounting operations but the actual work is financial analysis and reporting.
  >
  > Targeting established companies in Dubai. Salary expectation 5,000 to 7,000 AED, two months notice.
  >
  > CV attached. Happy to chat if anything is relevant.
- **Their response:** —
- **Next message:** —
- **Notes:** Emails 1-4 were revised after sending — original drafts had AI-style writing. Revised versions documented here for record.

---

### 2. Hensa Bhatia — Fin-Storm
- **Contact:** careers@fin-storm.com
- **Date sent:** 2026-08-23
- **Status:** 🟡 PENDING
- **Follow-up date:** 2026-09-06 (2 weeks)
- **Message sent:**
  > Subject: CV Submission | Financial Analyst / FP&A | Dubai | Muhammed Ashnad K
  >
  > Hi Hensa,
  >
  > Reaching out to share my CV for Financial Analyst or FP&A roles in Dubai.
  >
  > Currently at Promotech Advertising where I handle monthly P&L reporting, variance analysis, management packs for the CEO, month end close and AP/AR across 475 accounts. I also ran the company's ERP migration from Tally to Odoo. Before that I was a Tax Analyst at EY in Bangalore. M.Sc. Financial Economics. Excel, Power BI, SQL and Python. Current title is accounting operations but the work maps directly to FP&A.
  >
  > Looking for established companies, Dubai based. Salary expectation 5,000 to 7,000 AED, two months notice.
  >
  > CV attached. Happy to connect if any of your current roles are relevant.
- **Their response:** —
- **Next message:** —

---

### 3. Nupur — Future Tense HR
- **Contact:** nupur@futuretensehr.com
- **Date sent:** 2026-08-23
- **Status:** 🟡 PENDING
- **Follow-up date:** 2026-09-06 (2 weeks)
- **Message sent:**
  > Subject: CV | Financial Analyst / FP&A | Dubai
  >
  > Hi Nupur,
  >
  > I'm a finance professional in Dubai with a Big Four background (EY) and an M.Sc. in Financial Economics, looking to move into a Financial Analyst or FP&A role.
  >
  > At Promotech Advertising I manage financial reporting, month end close and led a Tally to Odoo ERP migration from start to finish. Before that I was at EY in Bangalore as a Tax Analyst. Excel, Power BI, SQL and Python. The title is accounting operations but the actual work is financial analysis and reporting, which is why I'm making this move.
  >
  > Dubai based roles at established companies. Salary expectation 5,000 to 7,000 AED, two months notice.
  >
  > CV attached. Let me know if this is a profile you're currently working with.
- **Their response:** —
- **Next message:** —

---

### 4. Maleek — Talent Source Consults
- **Contact:** maleek@talentsourceconsults.com
- **Date sent:** 2026-08-23
- **Status:** 🟡 PENDING
- **Follow-up date:** 2026-09-06 (2 weeks)
- **Message sent:**
  > Subject: CV | Financial Analyst / FP&A | Dubai
  >
  > Hi Maleek,
  >
  > Wanted to reach out and send my CV across for Financial Analyst or FP&A roles in Dubai.
  >
  > EY background (Tax Analyst, Bangalore), M.Sc. Financial Economics, currently at Promotech Advertising running financial reporting, month end close and a full ERP migration I led from planning to go live. Excel, Power BI, SQL, Python. Current title is accounting operations but the analytical side is where I spend most of my time and where I want to go full time.
  >
  > Established companies in Dubai. Salary expectation 5,000 to 7,000 AED, two months notice.
  >
  > CV attached. Let me know if there's a fit.
- **Their response:** —
- **Next message:** —

---

### 5. Brian Casey — Cooper Fitch
- **Contact:** brian.casey@cooperfitch.ae
- **Date sent:** —
- **Status:** ⬜ NOT YET SENT
- **Follow-up date:** —
- **Message sent:**
  > Subject: CV | Financial Analyst / FP&A | Dubai
  >
  > Hi Brian,
  >
  > I came across your finance desk at Cooper Fitch and wanted to send my CV across. Looking for Financial Analyst or FP&A roles in Dubai.
  >
  > Currently at Promotech Advertising handling financial reporting, variance analysis, management packs for the CEO and month end close. I also ran the company's ERP migration from Tally to Odoo. Before this I was at EY in Bangalore as a Tax Analyst. M.Sc. Financial Economics, work in Excel, Power BI, SQL and Python. My title is accounting operations but the day to day work is financial analysis and reporting.
  >
  > Targeting established companies in Dubai. Salary expectation 5,000 to 7,000 AED, two months notice.
  >
  > CV attached. Does this fit anything you have on at the moment?
- **Their response:** —
- **Next message:** —
- **Notes:** Managing Consultant, Accounting & Finance at Cooper Fitch. Specialises in financial analysis, planning, budgeting, reporting across all UAE industries. CPA-qualified himself — understands finance backgrounds well.

---

### 6. Darius Dosieah — Robert Walters UAE
- **Contact:** darius.dosieah@robertwalters.com
- **Date sent:** —
- **Status:** ⬜ NOT YET SENT
- **Follow-up date:** —
- **Message sent:**
  > Subject: CV | Financial Analyst / FP&A | Dubai
  >
  > Hi Darius,
  >
  > Looking to move into a Financial Analyst or FP&A role in Dubai and wanted to reach out to your team at Robert Walters.
  >
  > Background: Tax Analyst at EY Bangalore, now at Promotech Advertising in Dubai where I run financial reporting, month end close and led a full ERP migration from Tally to Odoo. M.Sc. Financial Economics. Excel, Power BI, SQL and Python. Current title is accounting operations, which I know reads differently on a CV, but the day to day work is financial analysis and reporting.
  >
  > Targeting Dubai based roles at established companies. Salary expectation 5,000 to 7,000 AED, two months notice.
  >
  > CV attached. Happy to have a conversation if anything is relevant.
- **Their response:** —
- **Next message:** —
- **Notes:** Consultant (not Manager) — more accessible. Accounting & Finance Middle East desk at Robert Walters. Previously UK-based, now GCC. General inbox also confirmed: dubai@robertwalters.com.

---

### 7. Morgan Tull — Hays UAE
- **Contact:** morgan.tull@hays.com
- **Date sent:** —
- **Status:** ⬜ NOT YET SENT
- **Follow-up date:** —
- **Message sent:**
  > Subject: CV | Financial Analyst / FP&A | Dubai
  >
  > Hi Morgan,
  >
  > I understand you work with Finance Analyst placements in Dubai. Wanted to reach out and send my CV across.
  >
  > EY background (Tax Analyst, Bangalore), M.Sc. Financial Economics, currently running financial reporting, variance analysis, management packs and month end close at Promotech Advertising in Dubai. I also led the company's ERP migration from Tally to Odoo. Excel, Power BI, SQL and Python. Current title is accounting operations but looking to move into a dedicated financial analysis or FP&A role.
  >
  > Salary expectation 5,000 to 7,000 AED, two months notice. Dubai based roles only.
  >
  > CV attached. Anything on your current book that could be relevant?
- **Their response:** —
- **Next message:** —
- **Notes:** Specialist Consultant at Hays UAE — search results specifically identified him as managing Finance Analyst positions. Email follows Hays global format (firstname.lastname@hays.com).

---

## COMMENTS LOG

*(Record posts you commented on — useful to follow up if the poster engages)*

| Date | Person / Post | Platform | Comment left | Response? |
|---|---|---|---|---|
| — | — | — | — | — |

---

## HOW TO UPDATE THIS FILE

**When you get a response:**
> Tell Claude: "Théodore replied — he said [paste their message]"
> Claude will read the Notes section, draft a reply, and update the status.

**When you send a recruiter email:**
> Tell Claude: "Sent Talha email — [paste what you sent]"
> Claude will log it and set a follow-up reminder.

**When a conversation goes cold:**
> After 2 weeks with no response → status changes to ❌ NO RESPONSE
> Claude will suggest a follow-up nudge if appropriate.
